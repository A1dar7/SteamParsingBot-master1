...
def collect_data(SLOT_ID, SLOT_NAME, START=0, COUNT=1, COUNTRY='RU', CURRENCY='RUB'):
    ...
    URL = f'{STEAM}/{SLOT_ID}/{SLOT_NAME}/{QUERY}'
    ...
    for data in datas['listinginfo']:
        print(price_normalize(str(datas['listinginfo'][data]['converted_fee_per_unit']),
                              str(datas['listinginfo'][data]['converted_price_per_unit'])))
        for data in datas['assets'][str(SLOT_ID)]['2']:
            print(data)
            print(datas['assets'][str(SLOT_ID)]['2'][data]['icon_url_large'])
            print(datas['assets'][str(SLOT_ID)]['2'][data]['name'])
            print(datas['assets'][str(SLOT_ID)]['2'][data]['type'])
            print(datas['assets'][str(SLOT_ID)]['2'][data]['descriptions'][0]['value'])
